...
def hero():
    url = 'http://api.steampowered.com/IEconDOTA2_570/GetHeroes/v1/?format=JSON&language=en_us&key=3A99C0D9D17ADD29409AB7700C6E9F6F'
    response = None
    connect = None 
    r = None 
    try:
    ...
    try:
        ...
        if r is not None:
            hero = {}
            for i in r['result']['heroes']:
                hero[i['localized_name']] = int(i['id'])
            return hero
